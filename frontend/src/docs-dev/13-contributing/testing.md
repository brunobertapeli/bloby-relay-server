...
Morphy does not currently have an automated test suite. All testing is manual.
...
- Set tunnel mode to `quick` via `morphy init` or by editing `~/.morphy/config.json`
- Start Morphy and confirm the `*.trycloudflare.com` URL appears in logs
...
- Start Morphy and confirm the named tunnel connects
...
Morphy runs on Windows, macOS, and Linux (including Raspberry Pi). Key areas that differ:
...
- **Data directory:** `~/.morphy/` on all platforms (via `os.homedir()`).
- **Daemons:** On Linux/macOS, Morphy supports systemd/launchd daemon mode. Windows uses Task Scheduler.
