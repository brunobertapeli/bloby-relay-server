...
The dashboard Vite config maps `@` to `workspace/client/src`. The Morphy chat Vite config maps `@` to `supervisor/chat/src`. These are independent alias namespaces, one per build target.
