...
In development, only the dashboard Vite server runs as a dev server (with HMR attached to the supervisor's HTTP server). The Morphy chat UI is pre-built and served as static files.
