...
| Morphy Relay             | Relay server (cloud)    | HTTP over loopback |
...
When the Morphy Relay is in use, the relay server at `api.morphyagent.com` terminates TLS for the `*.morphyagent.com` domain. It then re-encrypts traffic to forward it through the Cloudflare Tunnel URL (which is itself HTTPS).
