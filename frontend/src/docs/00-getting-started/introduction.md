---
title: Introduction
---

# Introduction

Welcome to Morphy, your personal AI agent inside your own personal app.

Think of it as an AI agent like OpenClaw, but instead of living in your terminal, Morphy lives in its own full-stack app. Get a managed instance (we host it for you on AWS), or install it on any machine that stays on (Mac Mini, VPS, Raspberry Pi). Either way, you get a URL you can visit from your phone, laptop, anywhere. Behind that URL is a PWA with a chat bubble (your direct line to the agent) and a workspace the agent builds and evolves through conversation.

## What makes Morphy different

Most AI coding agents give you a terminal or a chat window. Morphy gives the agent an entire application (frontend, backend, and database) that it shapes into whatever you need.

- **It's an agent and a playground.** Like OpenClaw, but with its own full-stack codebase to build anything in.
- **Pocket vibe coding.** Talk to your agent from your phone, by text or voice, and watch it build real software.
- **The chat never dies.** It runs in an isolated iframe, so even if the agent breaks the workspace you can always reach it and ask for fixes.
- **Mini apps on demand.** "I need a calorie counter." The agent builds one and adds it to your sidebar.
- **Managed or self-hosted.** Let us run it for you on AWS, or run it on your own hardware. Self-hosted bots connect through Morphy's built-in secure carrier, with no extra tunnel software and a stable URL.
- **It remembers you.** Memory files keep context across sessions.
- **It wakes up on its own.** Scheduled tasks and periodic pulses keep it proactive.

## How it works

Self-hosted:

1. Install Morphy on a machine that stays on
2. Run `morphy init`. It sets up everything, connects through the built-in carrier, and gives you a URL
3. Visit the URL from any device, complete onboarding (AI provider, password, 2FA)
4. Talk to it. It builds what you ask for. The workspace grows with every conversation.

Managed instances skip the install. Pick a handle, plan, and region on morphyagent.com, wait a few minutes while your instance provisions itself, then open `yourhandle.morphyagent.com` and complete the same onboarding. No CLI involved.
