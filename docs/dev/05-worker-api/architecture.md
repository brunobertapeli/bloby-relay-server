...
Browser / Relay
  ...
  Supervisor  (:3000)  -- proxies /api/* -->  Worker  (:3001)
                                                 |
                                                 v
                                         ~/.fluxy/memory.db   (SQLite, WAL mode)
                                         ~/.fluxy/config.json  (BotConfig)
...
1. `loadConfig()` reads `~/.fluxy/config.json`.
2. `initDb()` opens (or creates) `~/.fluxy/memory.db`, runs the schema DDL,
