...
function initWebPush() {
  ...
  webpush.setVapidDetails('mailto:push@morphyagent.com', publicKey, privateKey);
}
```

The `mailto:` subject is set to `push@morphyagent.com`. VAPID keys use the `web-push` library's `generateVAPIDKeys()` which produces an ECDSA P-256 key pair.
