...
Morphy uses **SQLite** as its embedded relational database, accessed through the
...
aligns with Morphy's self-hosted philosophy -- there is no external database
...
|---|---|---|
| **Primary (memory.db)** | `~/.morphy/memory.db` | Conversations, messages, settings, auth sessions, push subscriptions, trusted devices |
