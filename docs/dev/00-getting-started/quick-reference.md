...
```plain
bin/            CLI entry point (bloby init/start/stop/status)
...
dist-bloby/     Pre-built chat UI (do not edit directly)
```

**Runtime data:**

```plain
~/.bloby/config.json    User configuration
~/.bloby/memory.db      SQLite database
~/.bloby/workspace/     Deployed workspace copy
```
