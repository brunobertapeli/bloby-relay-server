...
Bloby has a strict separation between four top-level directories, plus a built output directory. Every file should live in exactly one of these areas.
...
| `supervisor/bloby-agent.ts` | Claude Agent SDK wrapper, system prompt injection, memory file loading |
...
| `worker/prompts/bloby-system-prompt.txt` | The agent's system prompt template |
...
The chat interface is a separate React SPA that compiles to `dist-bloby/`. It runs inside an iframe on the dashboard, fully isolated from the main application.

| File | Role |
|---|---|
| `supervisor/chat/bloby.html` | Chat entry point HTML |
| `supervisor/chat/bloby-main.tsx` | Chat app bootstrap, WebSocket connection, wizard integration |
...
### `dist-bloby/` -- Built Chat Output

Pre-built static files for the chat SPA. Served by the supervisor at `/bloby/*`.

**Rule: Never edit `dist-bloby/` directly.** It is generated by `vite build --config vite.bloby.config.ts` from `supervisor/chat/`. Run `npm run build:bloby` to rebuild it.
