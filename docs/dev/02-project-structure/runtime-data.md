...
When installed globally (`npm install -g morphyagent`) or via the curl installer, Morphy copies its source to `~/.morphy/` and operates from there. This directory also stores all runtime data.
...
~/.morphy/
  ...
  dist-chat/               Pre-built chat UI (overwritten on upgrade)
  ...
  vite.chat.config.ts      Chat Vite config (overwritten on upgrade)
