...
Fluxy uses WebSockets for two distinct purposes: the Fluxy chat interface and Vite Hot Module Replacement (HMR).

### 6.1 Fluxy Chat WebSocket

The chat WebSocket is served at the path `/fluxy/ws`. It is implemented as a `noServer` WebSocket using the `ws` library:
...
server.on('upgrade', async (req, socket, head) => {
  if (!req.url?.startsWith('/fluxy/ws')) {
    return; // Let Vite handle non-Fluxy upgrades
  }
  // Auth check...
...
If portal auth is configured, the WebSocket connection requires a `token` query parameter (`/fluxy/ws?token=...`). The token is validated against the worker before the upgrade completes. Invalid or missing tokens result in `401 Unauthorized` and socket destruction.
...
- If the URL starts with `/fluxy/ws`, it is handled by the Fluxy chat WebSocket.
...
No explicit `clientPort` is configured for HMR. The comment in `vite-dev.ts` explains: "the browser connects on the same origin the page is served from, so it works both locally (localhost:3000) and through the relay (riven.fluxy.bot:443)."
