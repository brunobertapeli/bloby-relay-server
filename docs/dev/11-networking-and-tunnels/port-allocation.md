...
The base port is stored in `~/.morphy/config.json` (the path resolved by `shared/paths.ts`). The config file is loaded by `loadConfig()` and written by `saveConfig()` in `shared/config.ts`. The default value in the `DEFAULTS` constant is `3000`:
