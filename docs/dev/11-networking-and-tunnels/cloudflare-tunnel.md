...
Before starting a tunnel, Morphy ensures the `cloudflared` binary is available. The `findBinary()` function checks two locations:

1. **System-wide install** -- runs `which cloudflared` (or `where cloudflared` on Windows). If found, returns `'cloudflared'` (uses the PATH).
2. **Local install** -- checks `paths.cloudflared` (resolved by `shared/paths.ts` to `~/.morphy/bin/cloudflared` or `cloudflared.exe` on Windows). Validates the file size is at least 10 MB (`MIN_CF_SIZE = 10 * 1024 * 1024`) to reject truncated downloads. If the file exists but is too small, it is deleted.
...
Quick tunnel URLs are ephemeral -- they change every time the tunnel restarts. This is why the Morphy Relay exists: it provides a stable domain that re-maps to the new ephemeral URL.
