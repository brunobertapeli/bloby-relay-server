...
Fluxy does not handle TLS itself. The TLS termination points are:
...
| Fluxy Relay             | Relay server (cloud)    | HTTP over loopback |
...
When the Fluxy Relay is in use, the relay server at `api.fluxy.bot` terminates TLS for the `*.fluxy.bot` domain. It then re-encrypts traffic to forward it through the Cloudflare Tunnel URL (which is itself HTTPS).
