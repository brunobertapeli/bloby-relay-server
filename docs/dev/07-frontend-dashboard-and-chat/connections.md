...
Browser request to :3000
    |
    +-- /fluxy/widget.js    --> Serve widget.js directly from supervisor/
    +-- /sw.js, /fluxy/sw.js --> Serve embedded service worker constant
    +-- /app/api/*           --> Proxy to user's backend server (:3004)
    +-- /api/*               --> Proxy to worker process (:3001)
    +-- /fluxy/*             --> Serve static files from dist-fluxy/
...
All requests that do not match `/api/*`, `/app/api/*`, `/fluxy/*`, `/sw.js`, or `/fluxy/widget.js` are forwarded to the dashboard's Vite dev server. This includes the initial HTML page load (`/`), JavaScript modules, CSS files, static assets, and source maps.
...
Requests to `/fluxy/*` are served as static files from the pre-built `dist-fluxy/` directory. The URL prefix `/fluxy/` is stripped before resolving the file path. Security: a directory traversal check ensures the resolved path stays within `dist-fluxy/`.

Cache headers differ by file type:
- **HTML files** (`fluxy.html`, `onboard.html`): `Cache-Control: no-cache` so rebuilds are picked up immediately.
...
1. **Backdrop** (`#fluxy-widget-backdrop`) -- A semi-transparent overlay behind the panel.
2. **Panel** (`#fluxy-widget-panel`) -- A fixed-position panel (480px wide, full height) that slides in from the right. Contains an iframe pointing to `/fluxy/` (the chat app).
3. **Bubble** (`#fluxy-widget-bubble`) -- A 60px circular button in the bottom-right corner. Shows an animated `.webm` video on non-Safari browsers, and a static `.png` on Safari (which lacks WebM alpha support).
...
- **`fluxy:close`** -- Closes the panel (user tapped the back arrow in chat).
- **`fluxy:install-app`** -- Triggers the PWA install prompt. If `beforeinstallprompt` was captured, calls `prompt()`. Otherwise, sends `fluxy:show-ios-install` back to the iframe so the chat can show manual iOS instructions.

### Onboarding Integration

On load, the widget fetches `/api/settings` to check `onboard_complete`. If onboarding is not complete, it hides the chat bubble entirely. It listens for `fluxy:onboard-complete` from the onboarding iframe to re-show the bubble.
...
However, the chat iframe itself is not affected by dashboard HMR because it is served from `dist-fluxy/` (static files). The iframe's WebSocket connection, message history, and streaming state survive a full dashboard page reload.
...
The supervisor handles WebSocket upgrades by letting Vite handle any upgrade request that does not match `/fluxy/ws`:
...
The chat app has no HMR and no dev server in production. It is pre-built into `dist-fluxy/` and served as static files. To update the chat UI:

1. Modify source files in `supervisor/chat/`.
2. Run `npm run build:fluxy`.
3. The new `dist-fluxy/` files are served immediately (HTML has `no-cache` headers).
...
2. **Fluxy Chat WebSocket** -- Handled by a `WebSocketServer` instance (`fluxyWss`). URL path: `/fluxy/ws`.
...
Chat WebSocket connections require authentication (if a portal password is configured). The auth token is passed as a query parameter: `/fluxy/ws?token=...`. The supervisor validates it against the worker's `/api/portal/validate-token` endpoint (with a 60-second TTL cache).
...
3. The returned JWT is stored in `localStorage` (key `fluxy_token`) and sent as `Authorization: Bearer` on all API requests and as `?token=` on WebSocket connections.
