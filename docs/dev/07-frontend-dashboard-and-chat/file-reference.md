...
| `Sidebar.tsx` | 256px sidebar with Morphy branding, time-aware greeting, and navigation (Dashboard, My Apps dropdown, Reports, Research, What Else). Uses `lucide-react` icons and sidebar-specific color tokens. |
...
| `onboard.html` | Onboarding wizard HTML shell. Same structure as `chat.html` but loads `onboard-main.tsx`. |
