...
After `morphy init`, the chat bubble opens with a step-by-step wizard. This is where you connect an AI provider and set up remote access.
...
Claude is recommended — it uses the Claude Agent SDK under the hood, which gives Morphy full tool access (reading, writing, editing files, running commands). OpenAI works too, but with simpler chat-only capabilities.
...
### 5. Handle (optional)
Register a public username with the Morphy Relay. This gives you a permanent URL like:
- `username.open.morphyagent.com` (free)
...
### 8. Voice (optional)
Add an OpenAI API key for Whisper voice transcription. This lets you send voice messages to Morphy.
